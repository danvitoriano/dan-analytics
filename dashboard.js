  ...
  const rows = sorted.map(p => {
    ...
    return \`<tr class="\${isHot ? 'highlight' : ''}">
      ...
      <td class="col-link" style="width:180px">
        \${p.permalink ? \`<a href="\${p.permalink}" target="_blank">\${p.permalink}</a>\` : '—'}
        \${p.id ? \` <a href="https://business.facebook.com/latest/posts/published_posts?asset_id=17841400529260114&post_id=\${p.id}" target="_blank" title="Abrir no Meta Business Suite" style="color:#555;font-size:14px;vertical-align:middle;text-decoration:none">↗</a>\` : ''}
      </td>
